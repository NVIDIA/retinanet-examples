...
We provide two ways to do inference with `retinanet`:
...
`retinanet infer` will run distributed inference across all available GPUs. When using PyTorch, the default behavior is to run inference with mixed precision. The precision used when running inference with a TensorRT engine will correspond to the precision chosen when the model was exported to TensorRT (see [TensorRT section](#exporting-trained-pytorch-model-to-tensorrt) below). 
...
```bash
retinanet export model.pth model_fp16.plan --batch 1 --size 1280
```
...
```bash
retinanet export model.pth model_fp32.plan --full-precision --batch 1 --size 800 1280
```
...
```bash
retinanet export model.pth model_int8.plan --batch 2 --int8 --calibration-images /data/val/ --calibration-batches 10 --calibration-table model_calibration_table
```
...
```bash
retinanet export model.pth model_int8.plan --batch 2 --int8 --calibration-table model_calibration_table
```
...
```bash
retinanet export model.pth model.onnx --size 800 1280
```
