...
class ModelRotated(nn.Module):
    ...
    def __init__(self, backbones='ResNet50FPN', classes=80, config={}):
        ...
        anchors = len(self.ratios) * len(self.scales) * len(self.angles)
        self.cls_head = make_head(classes * anchors)
        self.box_head = make_head(6 * anchors)  # theta -> cos(theta), sin(theta)
    ...
    def __repr__(self):
        return '\n'.join([
            '     model: {}'.format(self.name),
            '  backbone: {}'.format(', '.join([k for k, _ in self.backbones.items()])),
            '   classes: {}, anchors: {}'.format(self.classes, len(self.ratios) * len(self.scales) * len(self.angles)),
        ])
    ...
    def initialize(self, pre_trained):
        if pre_trained:
            ...
            state_dict = self.state_dict()
            chk = torch.load(pre_trained, map_location=lambda storage, loc: storage)
            ignored = ['cls_head.8.bias', 'cls_head.8.weight', 'box_head.8.bias', 'box_head.8.weight']
            weights = {k: v for k, v in chk['state_dict'].items() if k not in ignored}
            state_dict.update(weights)
            self.load_state_dict(state_dict)
    ...
    def forward(self, x):
        ...
        # Perform non-maximum suppression
        decoded = [torch.cat(tensors, 1) for tensors in zip(*decoded)]
        return nms_rotated(*decoded, self.nms, self.detections)
    ...
    def _extract_targets(self, targets, stride, size):
        ...
        for target in targets:
            ...
            snapped = snap_to_anchors_rotated(
                target, [s * stride for s in size[::-1]], stride,
                self.anchors[stride], self.classes, targets.device)
